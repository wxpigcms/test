...
class RepastAction extends UserAction {
    ...
    public function _initialize() {
        ...
        IS_GET && ($this->_cid = isset($_GET['cid']) ? intval($_GET['cid']) : session('companyid'));
        IS_POST && ($this->_cid = isset($_POST['cid']) ? intval($_POST['cid']) : session('companyid'));
...
//        }
     ...
     * 商家设置
     */
    public function shangjia_set() {
        $shangjia_list = M('Dish_company')->select();
        $list = M('Company')->where(array('token' => $this->token, 'isbranch' => 2))->select();
        $dinings = $shangjia = array();
        foreach ($list as $l) {
            $dinings[$l['id']] = $l;
        }
        foreach ($shangjia_list as $k => $t) {
            if (!$dinings[$t['cid']]) {
                unset($shangjia_list[$k]);
            } else {
                $t['name'] = isset($dinings[$t['cid']]['name']) ? $dinings[$t['cid']]['name'] : '';
                $shangjia[] = $t;
            }
        }
        $this->assign('shangjia_list', $shangjia);
        $this->display();
    }
    ...
    public function shangjia_add() {
        //商家信息
        $shang_jia = M('Company');
        $shang_jia_info = $shang_jia->field('id,name')->where(array('token' => $this->token, 'isbranch' => 2))->select();
        $dataBase = D('Dish_company');

        $type = $this->_get('type', 'trim');
        $findData = $dataBase->where(array('cid' => $this->_cid))->find();
        if ($type == 'add') {
            $findData ? $this->error('该商家已经绑定过') : '';
        }
        if (IS_POST) {
            if ($findData) {//edit
                if ($dataBase->create() !== false) {
                    ...
                    if ($action != false) {
                        $this->success('修改成功', U('Repast/shangjia_set', array('token' => $this->token, 'cid' => $this->_cid)));
                    } else {
                        $this->error('操作失败');
                    }
                } else {
                ...
                if ($dataBase->create() !== false) {
                    $action = $dataBase->add();
                    if ($action != false) {
                        $this->success('添加成功', U('Repast/shangjia_set', array('token' => $this->token, 'cid' => $this->_cid)));
                    } else {
                        $this->error('操作失败');
                    }
                } else {
            ...
            $this->assign('shangjia', $shang_jia_info);
            $this->display();
        }
    }
    /**
    ...
    public function shangjia_edit() {
        ...
        if (IS_POST) {
            ...
            $this->assign('company', $findData);
            $this->assign('shangjia', $shang_jia_info);
            $this->assign('shangjia_id', $shangjia_id);
            $this->display();
        }
        $this->display();
    }
